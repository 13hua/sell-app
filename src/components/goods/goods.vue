<template>
  <div>
    <div class="goods">
      ...
      <shopcart ref="shopcartel" @click="addFood" :select-foods="selectFoods" :delivery-price="seller.deliveryPrice"
...
export default {
  ...
  methods: {
    ...
    selectFood(food, event) {
      ...
      this.$refs.food.show(); // 调用food自组件的方法
    },
    addFood(target) {
      this._drop(target);
    },
    _drop(target) {
      // 体验优化，异步执行下落动画
      this.$nextTick(() => {
        this.$refs.shopcartel.drop(target);
      });
    },
    _initScroll() {
